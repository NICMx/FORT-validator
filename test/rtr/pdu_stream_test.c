...
START_TEST(test_serial_query_from_stream)
{
	...
	ck_assert_uint_eq(false, pdustream_parse(stream, &reqs));
	ck_assert_uint_eq(1u, reqs.count);

	request = rtreqlist_pop(&reqs);
	...
	ck_assert_ptr_eq(NULL, rtreqlist_pop(&reqs));
...
START_TEST(test_error_report_from_stream)
{
	...
	stream = create_stream_fd(input, sizeof(input), RTR_V1);
	ck_assert_uint_eq(false, pdustream_parse(stream, &reqs));
	ck_assert_uint_eq(0, reqs.count);
...
START_TEST(test_multiple_pdus)
{
	...
	ck_assert_uint_eq(true, pdustream_parse(stream, &reqs));
	ck_assert_uint_eq(3u, reqs.count);
	...
	ck_assert_uint_eq(true, pdustream_parse(stream, &reqs));
	ck_assert_uint_eq(2u, reqs.count);
...
static void
ck_one_reset_query(struct rtr_request_list *list)
{
	struct rtr_request *request;

	request = rtreqlist_pop(list);
	ASSERT_RQ(request, RTR_V1);
	ck_assert_uint_eq(8, request->pdu.raw.bytes_len);
...
START_TEST(test_too_many_pdus)
{
	...
	ck_assert_int_eq(8, write(pipes[1], reset_query, sizeof(reset_query)));
	ck_assert_uint_eq(true, pdustream_parse(stream, &reqs));
	ck_assert_uint_eq(4u, reqs.count);

	ck_one_reset_query(&reqs);
	ck_assert_uint_eq(3u, reqs.count);
