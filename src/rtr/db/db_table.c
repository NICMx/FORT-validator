 ...
 * "validated_payload"? (Like VRP, minus "ROA")
 * "verified_payload"?
 */
struct db_table {
...
add_aspa(struct db_table *table, struct hashable_aspa *new)
{
	...
	pr_trc("Adding ASPA for customer %u", new->v->customer);
	...
	if (error) {
		pr_err("Cannot store ASPA: %s", strerror(error));
		return error;
	}
...
write_u8(FILE *file, uint8_t v)
{
	return (fwrite(&v, 1, 1, file) != 1)
	    ? pr_err("fwrite() could not write 1 byte.")
	    : 0;
}
...
write_u32(FILE *file, uint32_t v)
{
	...
	return (fwrite(&v, 4, 1, file) != 1)
	    ? pr_err("fwrite() could not write 4 bytes.")
	    : 0;
}
...
write_buf(FILE *file, unsigned char *buf, size_t len)
{
	return (fwrite(buf, len, 1, file) != 1)
	    ? pr_err("fwrite() could not write %zu bytes.", len)
	    : 0;
}
...
cache_metadata(struct db_table *table)
{
	...
	if (error) {
		pr_err("Could not save RTR metadata; RTR can no longer be served.");
		file_rm_rf("rtr");
	}
...
db_table_cache(struct db_table *table)
{
	...
	if (ret == ENOENT)
		...
		pr_err("Cannot access RTR index file: %s", strerror(ret));
		return ret;
	}

	serial = rtridx_add_serial(&table->rtr);

	ret = file_mkdir("rtr", true); /* cache/rtr */
	if (ret)
		return ret;

	ret = file_mkdir(rtr_filename3(path, serial), true); /* cache/rtr/1234 */
	if (ret)
		return ret;
