...
validate_rtr_version(struct rtr_request *request)
{
	...
	if (stream->rtr_version == -1) {
		...
		pr_err("%s: Unsupported RTR version: %u",
		    stream->addr, reqver);
	...
	if (stream->rtr_version != reqver) {
		pr_err("%s: Client changed RTR version: %u -> %u",
    ...
    struct aspa_providers *providers)
{
	...
	if (providers->count > MAX_ASPA_PROVIDERS)
		return pr_err("Customer has too many providers: %zu",
		    providers->count);
	...
	for (i = 0; i < providers->count; i++) {
		if (fread(buf, 4, 1, file) == 1) {
			...
			error = pr_err("File ended prematurely");
			goto end;
		} else if (ferror(file)) {
			error = errno;
			if (!error)
...
skip_providers(unsigned char const *hdr, FILE *file)
{
	...
	for (total = read_u32(hdr + 4); total > 0; total -= red) {
		...
		if (want != red) {
			if (feof(file))
				return pr_err("File ended prematurely");
			if (ferror(file)) {
				error = errno;
				if (!error)
					error = EINVAL;
...
handle_reset_query_pdu(struct rtr_request *request)
{
	...
	pr_trc("Reset Query. Request version: %u", request->pdu.rtr_version);
...
handle_serial_query_pdu(struct rtr_request *request)
{
	...
	pr_trc("Sending RTR delta: %u-%u", oserial, nserial);
