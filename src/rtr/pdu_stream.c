	...
	for (; in->end < in->start + RTRPDU_MAX_LEN2; in->end += consumed) {
		...
		if (consumed == -1) {
			...
			if (error == EAGAIN || error == EWOULDBLOCK) {
				...
				pr_err("Client socket read errored: %s",
				    strerror(error));
				return BS_ERROR;
			}
		}
		...
		if (consumed == 0) {
			pr_trc("Client '%s' closed the socket.", in->addr);
			return BS_EOS;
		}

		pr_trc("Received %zd bytes from %s.", consumed, in->addr);
	}
...
queue_request(struct rtr_request_list *list, struct rtr_request *req)
{
	...
	if (list->count >= 4) {
		pr_err("%s: Too many simultaneous requests; Dropping RTR connection.",
		    req->client_addr);
		return ENOSPC;
	}
...
{
	pr_trc("Shutting down input stream of client %s.", stream->addr);
	PS_ENABLE(stream, EOS);
	if (shutdown(stream->fd, SHUT_RD) < 0)
		pr_wrn("Can't shut down read end of client socket: %s",
		    strerror(errno));
}
