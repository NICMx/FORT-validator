...
handle_request(struct rtr_request *req)
{
	...
	default:
		...
		pr_panic("Unexpected PDU type: %u", req->pdu.type);
	}
...
handle_clients(void *arg)
{
	...
	while (true) {
		...
		pr_trc("RTR worker thread: Claimed client %s", client->addr);

		do {
			if (!pdustream_parse(client, &reqs)) {
kick_client:			pr_trc("RTR worker thread: %s errored.",
				    client->addr);
			...
			if (TAILQ_EMPTY(&reqs.nodes)) {
				pr_trc("RTR worker thread: %s done.",
				    client->addr);
				mutex_lock(&lock);
				PS_DISABLE(client, CLAIMED);
				break;
		...
		if (write(worker2poller[1], &one, 1) < 0)
			pr_wrn("Cannot wake up RTR server thread: %s",
			    strerror(errno));
	}
...
fddb_poll(void)
{
	...
	if (fd->revents & POLLIN) {
		if (read(fd->fd, trash, sizeof(trash)) < 0)
			pr_wrn("Cannot read RTR server wakeupper: %s",
			    strerror(errno));
	}
...
end_server_threads(size_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (error)
			pr_wrn("pthread_join: %s", strerror(error));
		pr_trc("Ended RTR server thread #%zu.", i + 1);
	}
...
rtr_start(void)
{
	...
	if (error) {
		pr_crit("pthread_cond_init(p2w) returned error %d: %s",
	...
	if (pipe(worker2poller) < 0) {
		error = errno;
		pr_crit("Cannot create RTR worker-poller pipe: %s",
		    strerror(error));
		goto cond;
	}
	...
	for (i = 0; i < config_get_thread_pool_server_max(); i++) {
		error = pthread_create(&server_threads[i], NULL,
		    handle_clients, NULL);
		if (error) {
			pr_crit("pthread_create() returned error %d: %s",
			    error, strerror(error));
			goto threads;
		}

		pr_trc("Spawned RTR server thread #%zu.", i + 1);
	}
